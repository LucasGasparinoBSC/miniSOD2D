	...
	use mod_constants
		...
		contains
				...
				subroutine full_convec_ijk_vari1(nelem,npoin,connec,Ngp,dNgp,He,gpvol,dlxigp_ip,xgp,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,u,q,rho,pr,E,Rmass,Rmom,Rener)
					...
					!$acc parallel loop gang private(ipoin,Re_ener,Re_mass,Re_mom,ul,ql,rhol,prl,El,fl,fel) present(connec,u,q,rho,pr,E,Rmass,Rmom,Rener)
					do ielem = 1,nelem
						!$acc loop vector
						do inode = 1,nnode
						...
						do idime = 1,ndime
							do jdime = 1,ndime
								do inode = 1,nnode
									fl(inode,idime,jdime)  = q(connec(ielem,inode),idime)*u(ipoin(inode),jdime)
								end do
							end do
						end do
						!$acc loop vector
				...
				end subroutine full_convec_ijk_vari1

				subroutine full_convec_ijk_vari2(nelem,npoin,connec,Ngp,dNgp,He,gpvol,dlxigp_ip,xgp,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,u,q,rho,pr,E,Rmass,Rmom,Rener)
					...
					do ielem = 1,nelem
						!$acc loop vector
						do inode = 1,nnode
							ipoin(inode) = connec(inode,ielem)
						end do
						!$acc loop vector collapse(2)
						do idime = 1,ndime
							do inode = 1,nnode
				...
				end subroutine full_convec_ijk_vari2
